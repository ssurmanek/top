...
# Import salt master/minion configuration
import salt.config
#Import wheel client:
import salt.wheel

# Import libs for idm extension:
import uuid
import datetime

#Import encription libs:
...
def salt_token_tool():
    ...
    authZ = cherrypy.request.headers.get('Authorization', None)

    # X-Auth-Token header trumps session cookie
    if x_auth:
        cherrypy.request.cookie['session_id'] = x_auth
    if authZ and 'Bearer ' in authZ:
        cherrypy.request.cookie['session_id'] = authZ.split(' ')[1]
    if authZ and 'OAuth ' in authZ:
        cherrypy.request.cookie['session_id'] = authZ.split(' ')[1]
...
def cors_tool():
    ...
    resp_head['Access-Control-Expose-Headers'] = 'GET, POST', 'PATCH'
...
def hypermedia_in():
    '''
    Unserialize POST/PATCH data of a specified Content-Type.
    ...
    if ( (cherrypy.request.method.upper() == 'POST' or cherrypy.request.method.upper() == 'PATCH')
...
def lowdata_fmt():
    ...
    if cherrypy.request.method.upper() != 'POST' and cherrypy.request.method.upper() != 'PATCH':
        return
...
class LowDataAdapter(object):
    ...
    def exec_lowstate(self, client=None, token=None):
        ...
        if cherrypy.request.config.get('tools.sessions.on', False) and cherrypy.session.locked:
...
class ScimLogin (LowDataAdapter):
    #@cherrypy.config(**{'tools.json_in.force': False})
    def __init__(self, *args, **kwargs):
        super(ScimLogin, self).__init__(*args, **kwargs)
      
        self.auth = salt.auth.Resolver(self.opts)

    def POST(self, **kwargs):
        cherrypy_opts = salt.config.client_config('/etc/salt/master')['rest_cherrypy']
        if isinstance(cherrypy.serving.request.lowstate, list):
            cherrypy.serving.request.lowstate[0]['eauth'] = cherrypy_opts['authentication']
        else:
            cherrypy.serving.request.lowstate['eauth'] = cherrypy_opts['authentication']
        
        session_id, token, perm = executeLogin(self, **kwargs)

        return {
            'access_token': session_id, 
            'instance_url': 'http://{0}/'.format(cherrypy_opts['instance_url'])
        }

class User (LowDataAdapter):
    '''
    ...
    cherrypy_opts = opts['rest_cherrypy']

    def construct_user_response(self, user_id=None, user_name=None, **kwargs):
        meta = {
            'resourceType' : 'User',
            'location' : 'http://{0}/v1/Users/{1}'.format(self.cherrypy_opts['instance_url'], user_id)
        }
        if 'created' in kwargs and kwargs['created']:
            meta['created'] = kwargs['created']
        if 'lastModified' in kwargs and kwargs['lastModified']:
            meta['lastModified'] = kwargs['lastModified']

        response = {
            'id' : user_id,
            #'externalId' : str(user_name),    #SCIM 1
            #'displayName' : str(user_name),
            'userName' : str(user_name),
            'meta' : meta
        }
        if 'family_name' in kwargs and kwargs['family_name'] and 'given_name' in kwargs and kwargs['given_name']:
            name = {
                'familyName' : str(kwargs['family_name']),
                'givenName' : str(kwargs['given_name'])
            }
            response['name'] = name
        if 'emails' in kwargs and kwargs['emails']:
            emails = []
            emails_dict=kwargs['emails']
            for email in emails_dict:
                email_dict = {'value':email, 'type':emails_dict[email]['type'], 'primary':emails_dict[email]['primary']}
                emails.append(email_dict)
            response['emails'] = emails
        if 'entitlements' in kwargs and kwargs['entitlements']:
            entitlements = []
            entitlements_list=kwargs['entitlements']
            for entitlement in entitlements_list:
                entitlement_dict = {'display':entitlement}
                entitlements.append(entitlement_dict)
            response['entitlements'] = entitlements
        if 'groups' in kwargs and kwargs['groups']:
            '''
            groups = []
            groups_list=kwargs['groups']
            for group in groups_list:
                group_dict = {'value':group}
                groups.append(group_dict)
            '''
            response['groups'] = self.parse_groups(kwargs['groups'])
    ...
    def get_filepath (self, file_name):
        ...
        if salt_config:
            # Cherrypy Config.
            file_roots = salt_config.get('pillar_roots', None)
            if file_roots:
                # ACL Config.
                pillar_root_path = file_roots.get('base', None)[0]
        return os.path.join(pillar_root_path, file_name)

    def get_all_users(self):
        abs_state_file = self.get_filepath("abs.sls")
        with open(abs_state_file, 'r') as infile:
            users = yaml.load(infile)
        return users['users_absolute']

    def apply_state(self, entitlements):

        def check_minions(minion_id, entitlements):
            #chcek if processed minion is in list of minions=groups
            if not entitlements:
                return False
            for entitlement in entitlements:
                if entitlement in minion_id:
                    return True
            return False
        try:
            cherrypy.request.lowstate = [{'tgt_type': 'list', 'tgt': entitlements, 'fun': 'state.apply', }]
            minion_results_generator = self.exec_lowstate(client='local', token=cherrypy.session.get('token'))
            success = True
            minion_results = next(minion_results_generator)
            failed_minions = []
            #check failed minions:
            for minion in minion_results:
                #if check_minions(minion, entitlements):
                if minion in entitlements:
                    single_minion_result = minion_results[minion]
                    for state in single_minion_result:
                        #if a fatal error occured (i.e. syntax error in state file)
                        #the minion's response is formated to list, else it is dict of responses
                        if isinstance(single_minion_result, dict):
                            state_result = single_minion_result[state]
                            if not state_result['result']:
                                failed_minion = {'minion' : minion, 'message' : state_result['comment']}
                                failed_minions.append(failed_minion)
                        else:
                            failed_minion = {'minion' : minion, 'message' : 'Fatal error.'}
                            failed_minions.append(failed_minion)
            return failed_minions
        except Exception, e:
            raise cherrypy.HTTPError(500, str(e))

    def get_failed_minion_list(self, fails_list):
        minion_list = []
        for minion in fails_list:
            minion_list.append(minion['minion'])
        return minion_list

    def restore_state(self, userName, user_attrs, successful_minions, existence):
        '''
        This function reverts the account to state before operation execution.
        Reversion is applied to minions with successful operation result, only.
        Operations to execute:
           +--------------------------------------+
           | operation --> restorative operation  |
           |======================================|
           |   Create  --> Delete                 |
           |    Update --> Update without changes |
           |   Delete  --> Create                 |
           +--------------------------------------+
        Notice: It is assumed that value of 'existence' attribute should be
                'present' before operation create/update/delete failed.
        '''
        if existence == "absent":
            user_attrs['existence'] = "absent"
        else:
            user_attrs['existence'] = "present"
        user_attrs['entitlements'] = successful_minions
        #rewrite relative state file:
        self.write_rel_state(userName, user_attrs)
        cherrypy.request.lowstate = [{'tgt_type': 'list', 'tgt': successful_minions, 'fun': 'state.apply', }]
        minion_results_generator = self.exec_lowstate(client='local', token=cherrypy.session.get('token'))
        minion_results = next(minion_results_generator)
        #raise cherrypy.HTTPError(400, str(minion_results))

    
    def find_between(self, string, first, last, reverse=False):
        try:
            if reverse:
    ...
    def parse_path(self, path):
        attr = self.find_between(str(path), "", "[")
        subattr_for_filter = self.find_between(str(path), "[", " ")
        operator = self.find_between(str(path), " ", " ")
        value_for_filter = self.find_between(str(path), "\"", "\"")
        end_part = self.find_between(str(path), "]", "", "r")
        subattr_to_replace = self.find_between(end_part, ".", "", "r")
        if not attr:
            return {}
        else:
            return {
                'attr':attr, 
                'subattr_for_filter':subattr_for_filter, 
                'operator':operator, 
                'value_for_filter':value_for_filter, 
                'subattr_to_replace':subattr_to_replace
            }

    #get base group (minion_id):
    ...
    #get list of all base groups (minion_ids):
    def get_base_groups_list(self, groups):
        base_groups_list = []
        for group in groups:
            base_groups_list.append(self.get_base_group(group))
        return base_groups_list

    #get minion groups:
    def get_sub_groups(self, group):
        if ':' in group:
            return group.split(':', 1)[1].split('&')
        else:
            return None

    def parse_groups(self, groups):
        '''
        Parse list of groups in form: 'minion_name:list_of_groups'
        to list of dictionaries with form 'minion_name:single_group':
        '''

        parsed_groups = []
        for group in groups:
            base_group = self.get_base_group(group)
            sub_groups = self.get_sub_groups(group)
            if sub_groups == None:
                parsed_groups.append({'display' : base_group})
            else:
                for sub_group in sub_groups:
                    parsed_group = {'display' : base_group+':'+sub_group}
                    parsed_groups.append(parsed_group)
        return parsed_groups

    def write_rel_state(self, username, user_attrs):
        try:
            user = { 'users_relative' : {
                    str(username) : user_attrs
                }
            }
            rel_state_file = self.get_filepath(file_name="rel.sls")
            with open(rel_state_file, 'w+') as outfile:
                yaml.dump(user, outfile, default_flow_style=False)
        except OSError:
            raise cherrypy.HTTPError(500, 'Writing relative state to file failed.')

    def write_abs_state(self, all_users):
        try: 
            users = { 'users_absolute' : all_users}
            abs_state_file = self.get_filepath(file_name="abs.sls")
            with open(abs_state_file, 'w+') as outfile:
                yaml.dump(users, outfile, default_flow_style=False)
        except OSError:
            raise cherrypy.HTTPError(500, 'Writing absolute state to file failed')
    
    def get_enc_dec_values(self):
        BS = 16
        pad = lambda s: s + (BS - len(s) % BS) * chr(BS - len(s) % BS)
        unpad = lambda s : s[0:-ord(s[-1])]
        master_pub = self.wheel.cmd('key.finger_master')
        key = str(master_pub).replace(":", "")
        return BS, pad, unpad, key

    def encrypt(self, raw):
        BS, pad, unpad, key = self.get_enc_dec_values()
        raw = pad(raw)
    ...
    def GET(self, uid=None):
       ...
       if uid:
           ...
           for username in all_users:
               if all_users[username]['id'] == uid and all_users[username]['existence'] == "present":
                   user = all_users[username]
                   break
           if not user:
               raise cherrypy.HTTPError(404, 'User with id {0} does not exists.'.format(uid))

           ret = self.construct_user_response(
               user_id=user['id'], 
               user_name=username, 
               family_name=user.get('familyName', None), 
               given_name=user.get('givenName', None), 
               emails=user.get('emails', None), 
               groups=user.get('groups', None),
               entitlements=user.get('entitlements', None),
               created=user.get('created', None),
               lastModified=user.get('lastModified', None))
           ...
           for username in all_users:
               user = all_users[username]
               if user['existence'] == "present":
                   users_list.append(
                       self.construct_user_response(
                           user_id=user['id'], 
                           user_name=username, 
                           family_name=user.get('familyName', None), 
                           given_name=user.get('givenName', None), 
                           ...
                           lastModified=user.get('lastModified', None)))
           return {
               "totalResults": len(users_list), #SCIM 1
               "schemas":["urn:ietf:params:scim:schemas:core:2.0:User"],  #SCIM 1
    ...
    def POST(self):
        #try:
            lowstate = cherrypy.request.lowstate
            #read attributes from body:
            for chunk in lowstate:
                userName = chunk.get('userName', None)
                password = chunk.get('password', None)
                name = chunk.get('name', None)
                if name:
                    familyName = chunk['name'].get('familyName', None)
                    givenName = chunk['name'].get('givenName', None)
                emails = chunk.get('emails', None)
                entitlements = chunk.get('entitlements', None)
                #groups = chunk['groups']
            #check mandatory attributes:
            if not userName:
                raise cherrypy.HTTPError(400, 'Mandatory attribute userName is missing')
            if not entitlements:
                raise cherrypy.HTTPError(400, 'Mandatory entitlements is missing')
            #check if user already exists
            abs_state_file = self.get_filepath(file_name="abs.sls")
            if not os.path.isfile(abs_state_file):
                all_users = {}
            else:
                all_users = self.get_all_users()
                exists_present = False
                exists_absent = False
                for user_name in all_users:
                    if user_name == userName and all_users[user_name]['existence'] == "present":
                        exists_present = True
                        break
                    if user_name == userName and all_users[user_name]['existence'] == "absent":
                        exists_absent = True
                        break
                if exists_absent:
            ...
            #transform parsed attributes to yaml file format:
            datetime_now = datetime.datetime.now().isoformat()
            user_attrs = {
                'existence' : 'present', 
                'id' : user_uid, 
                'created' : str(datetime_now), 
                'lastModified' : str(datetime_now)
            }
            if name and givenName:
                user_attrs['givenName'] = str(givenName)
            if name and familyName:
                user_attrs['familyName'] = str(familyName)
            if password:
                user_attrs['password'] = str(password)
            if emails:
                emails_dict = {}
                for email in emails:
                    emails_dict[str(email['value'])] = {'primary': email.get('primary', False), 'type': str(email['type'])}
                user_attrs['emails'] = emails_dict
            if entitlements:
                entitlements_list = []
                for entitlement in entitlements:
                    entitlements_list.append(str(entitlement['display']))
                user_attrs['entitlements'] = entitlements_list
 
            #rewrite relative state file:
            self.write_rel_state(userName, user_attrs)
            #apply state:
            fails = self.apply_state(entitlements_list)
            if fails:
                #restore state of users and throw exception:
                failed_minions = self.get_failed_minion_list(fails)
                successful_minions = set(entitlements_list).difference(set(failed_minions))
                self.restore_state(userName, user_attrs, list(successful_minions), "absent")
                raise cherrypy.HTTPError(500, 'Operation failed for minion(s): {0}'.format(fails))
            #encode password before writting absolute state:
            if 'password' in user_attrs:
                user_attrs['password'] = self.encrypt(user_attrs['password'])#base64.b64encode(user_attrs['password'])
            #rewrite absolute state file:
            all_users.update({str(userName) : user_attrs})
            self.write_abs_state(all_users)
        #except Exception, e:
            #raise cherrypy.HTTPError(500, str(e))
            cherrypy.response.status = 201
            return self.construct_user_response(
                user_id=user_uid, 
                user_name=userName, 
        ...
        if uid:
            #try:
                ...
                for username in all_users:
                    ...
                    user_attrs = dict()
                    if all_users[username]['id'] == uid and all_users[username]['existence'] == "present":
                        #update value of attribute existence instead of deleting whole user:
                        user_attrs = all_users[username]
                        user_attrs['existence'] = "absent"
                        break
                if not user_attrs:
                    raise cherrypy.HTTPError(404, 'User with id {0} does not exists.'.format(uid))
                ...
                self.write_rel_state(username, user_attrs)
                #apply state:
                entitlements = user_attrs.get('entitlements', None)
                
                fails = self.apply_state(entitlements)
                if fails:
                    #restore state of users and throw exception:
                    failed_minions = self.get_failed_minion_list(fails)
                    successful_minions = set(entitlements).difference(set(failed_minions))
                    #decrypt password before restoring state:
                    if 'password' in user_attrs:
                        user_attrs['password'] = self.decrypt(user_attrs['password'])
                    self.restore_state(username, user_attrs, list(successful_minions), "present")
                    #cherrypy.request.handler = None # stop request
                    #cherrypy.response.status = 403 
                    #cherrypy.response.body = {"Operation failed for minion(s): ":fails}
                    #cherrypy.response.finalize()
                    #return {"Operation failed for minion(s): ":fails}
                    raise cherrypy.HTTPError(500, 'Operation failed for minion(s): {0}'.format(fails))
                #rewrite absolute state file:
                self.write_abs_state(all_users)
            ...
            raise cherrypy.HTTPError(400, 'User ID is not provided.')

    def PATCH(self, uid=None):

        def remove(parsed_path, user_attrs):
            if parsed_path:#"[" in path:
                attr = parsed_path['attr']#self.find_between(str(path), "", "[")
                subattr = parsed_path['subattr_for_filter']#self.find_between(str(path), "[", " ")
                operator = parsed_path['operator']#self.find_between(str(path), " ", " ")
                value = parsed_path['value_for_filter']#self.find_between(str(path), "\"", "\"")
                if operator != "eq":
                    raise cherrypy.HTTPError(406, 'operator \"{0}\" is not supported, acceptable operators: eq'.format(operator))
                if attr == "emails" and subattr == "value":
                    user_attrs['emails'].pop(value)
                    if len(user_attrs['emails']) == 0:
                        user_attrs.pop('emails')
                elif attr == "entitlements" and subattr == "display":
                    user_attrs['entitlements'].remove(value)
                    if len(user_attrs['entitlements']) == 0:
                        user_attrs.pop('entitlements')
                else:
                    user_attrs.pop(path)
            return user_attrs

        def replace(path, parsed_path, user_attrs):
            attrs_to_replace = {}
            if not path:
                attrs_to_replace = value
            if not parsed_path:#"[" not in path:
                attrs_to_replace = {str(path) : value}
            #if attrs_to_replace:
                for attr in attrs_to_replace:
                    if attr == "emails":
                        emails = {"emails" : {}}
                        for email in attrs_to_replace["emails"]:
                            emails['emails'][str(email['value'])] = {'primary': email.get('primary', False), 'type': str(email['type'])}
                        user_attrs.update(emails)
                    elif attr == "entitlements":
                        entitlements = {'entitlements' : []}
                        for entitlement in attrs_to_replace["entitlements"]:
                            entitlements['entitlements'].append(entitlement['display'])
                        user_attrs.update(entitlements)
                    else:
                        attr_dict = {str(attr) : str(attrs_to_replace[attr])}
                        user_attrs.update(attr_dict)
            if parsed_path:#"[" in path:
                attr = parsed_path['attr']#self.find_between(str(path), "", "[")
                subattr_for_filter = parsed_path['subattr_for_filter']#self.find_between(str(path), "[", " ")
                operator = parsed_path['operator']#self.find_between(str(path), " ", " ")
                value_for_filter = parsed_path['value_for_filter']#self.find_between(str(path), "\"", "\"")
                #end_part = self.find_between(str(path), "]", "", "r")
                subattr_to_replace = parsed_path['subattr_to_replace']#self.find_between(end_part, ".", "", True)
                if operator != "eq":
                    raise cherrypy.HTTPError(406, 'Operator \"{0}\" is not supported, acceptable operators: eq'.format(operator))
                elif attr == "emails" and subattr_for_filter == "value":
                    if value_for_filter not in user_attrs['emails'].keys():
                        raise cherrypy.HTTPError(400, 'User\'s emails does not contain email {0}'.format(value_for_filter))
                    if subattr_to_replace:
                        if subattr_to_replace in ["type", "primary"]:
                            user_attrs['emails'][value_for_filter][subattr_to_replace]=value
                        if subattr_to_replace == "value":
                            email = user_attrs['emails'][value_for_filter]
                            user_attrs['emails'].pop(value_for_filter)
                            user_attrs['emails'][value]=email
                    else:
                        user_attrs['emails'].pop(value_for_filter)
                        user_attrs['emails'][str(value['value'])] = {'primary': value.get('primary', False), 'type': str(value['type'])}
                elif attr == "entitlements" and subattr_for_filter == "display":
                    if subattr_to_replace and subattr_to_replace == "display":
                        entitlement=value
                    elif not subattr_to_replace:
                        entitlement = value['display']
                    else:
                        raise cherrypy.HTTPError(406, 'Replacing of attribute \"{0}\" is not supported, accepted attributes: display'.format(subattr_to_replace))
                    if value_for_filter in user_attrs['entitlements']:
                        user_attrs['entitlements'].remove(value_for_filter)
                        user_attrs['entitlements'].append(entitlement)
                    else:
                        raise cherrypy.HTTPError(400, 'User does not have entitlement {0}'.format(value_for_filter))
                else:
                    raise cherrypy.HTTPError(406, 'Replacing of specified attribute and subattribute combinations is not supported, accepted combinations: email-value, entitlement-display')
            return user_attrs

        def add(path, user_attrs):
            if path:
                attrs_to_add = {str(path) : value}
            else:
                attrs_to_add = value
            for attr in attrs_to_add:
                if attr == "emails":
                    if "emails" not in user_attrs:
                        user_attrs['emails'] = {}
                    for email in attrs_to_add["emails"]:
                        user_attrs['emails'][str(email['value'])] = {'primary': email.get('primary', False), 'type': str(email['type'])}
                elif attr == "entitlements":
                    if "entitlements" not in user_attrs:
                        user_attrs['entitlements'] = []
                    for entitlement in attrs_to_add["entitlements"]:
                        user_attrs['entitlements'].append(entitlement['display'])
                else:
                    user_attrs[str(attr)] = attrs_to_add[str(attr)]
            return user_attrs

        try:
            #chceck body and uuid:
            lowstate = cherrypy.request.lowstate
            if not lowstate:
                raise cherrypy.HTTPError(400, 'Request body is missing.')
            if not uid:
                raise cherrypy.HTTPError(400, 'User ID is not provided.')
            ...
            for username in all_users:
                if all_users[username]['id'] == uid and all_users[username]['existence'] == "present":
                    ...
                    if 'password' in user_attrs:
                        user_attrs['password'] = self.decrypt(user_attrs['password']) #base64.b64decode(user_attrs['password'])
                    user_attrs_copy = user_attrs.copy()
                    user_attrs['lastModified'] = str(datetime_now)
                    break
            if not user_attrs:
                raise cherrypy.HTTPError(404, 'User with id {0} does not exists.'.format(uid))
            #parse body:
            for chunk in lowstate:
                ...
                for operation in operations: 
                    ...
                    if not op or op not in ["add", "replace", "remove"]:
                        raise cherrypy.HTTPError(400, 'Mandtory "op" attribute is missing or its value is incorrect.')
                    path = operation.get('path', None)
                    parsed_path = self.parse_path(path)
                    if not path and op == "remove":
                        raise cherrypy.HTTPError(400, 'noTarget - "path" attribute is mandatory for remove operation.')
                    value = operation.get('value', None)
                    if not value and op != "remove":
                        raise cherrypy.HTTPError(400, '"value" attribute is mandatory for add and replace operation.')
                    #execute requested operation:
                    if op == "add":
                        user_attrs = add(path, user_attrs)
                    if op == "replace":
                        user_attrs = replace(path, parsed_path, user_attrs)
                    if op == "remove":
                        user_attrs = remove(parsed_path, user_attrs)
            #rewrite relative state file:
            self.write_rel_state(username, user_attrs)
            #apply state:
            fails = self.apply_state(user_attrs.get('entitlements', None))
            if fails:
                #restore state of users and throw exception:
                failed_minions = self.get_failed_minion_list(fails)
                successful_minions = set(user_attrs_copy.get('entitlements', [])).difference(set(failed_minions))
                self.restore_state(username, user_attrs_copy, list(successful_minions), "present")
                raise cherrypy.HTTPError(500, 'Operation failed for minion(s): {0}'.format(failes))
            #encode password before writing absolute state:
            if 'password' in user_attrs:
                user_attrs['password'] = self.encrypt(user_attrs['password']) #base64.b64encode(user_attrs['password'])
            #update and rewrite absolute state file:
            all_users.update({username : user_attrs})            
            self.write_abs_state(all_users)
        ...
        return self.construct_user_response(
            ...
            user_name=username,
            family_name=user_attrs.get('familyName', None),
            given_name=user_attrs.get('givenName', None),
            emails=user_attrs.get('emails', None),
            groups=user_attrs.get('groups', None),
            entitlements=user_attrs.get('entitlements', None), 
            created=user_attrs.get('created', None),
            lastModified=datetime_now
        )           
import contextlib
from cherrypy.lib import httputil as _httputil
class HTTPError(cherrypy.CherryPyException):
    ...
    def __init__(self, status=500, message=None):
        ...
        cherrypy.CherryPyException.__init__(self, status, message)
    ...
    def set_response(self):
        ...
        response.body = json.dumps(self._message)
...
class Schemas(LowDataAdapter):
    ...
    def GET(self, schema_id=None):
      try:
        ...
        user_account_attrs.append(user_name)
        ...
        name_sub_attrs.append(family_name)
        given_name = {
            "name" : "givenName",
            "type" : "string",
            "multiValued" : False,
        ...
        name_sub_attrs.append(given_name)
        name = {
            "name" : "name",
            "type" : "complex",
            "multiValued" : False,
            ...
            "required" : False,
            "subAttributes" : name_sub_attrs,
            "mutability" : "readWrite",
            "returned" : "default",
            "uniqueness" : "none"
        }
        user_account_attrs.append(name)
        ...
        #user_account_attrs.append(password)   #SCIM 1
        ...
        emails_sub_attrs.append(email_value)
        email_type = {
            "name" : "type",
            "type" : "string",
            "multiValued" : False,
        ...
        emails_sub_attrs.append(email_type)
        email_primary = {
            "name" : "primary",
            "type" : "boolean",
            "multiValued" : False,
        ...
        emails_sub_attrs.append(email_primary)
        emails = {
            "name" : "emails",
            "type" : "complex",
            "multiValued" : True,
            "description" : "Email addresses for the user.  The value SHOULD be canonicalized by the service provider, e.g., 'bjensen@example.com' instead of 'bjensen@EXAMPLE.COM'. Canonical type values of 'work', 'home', and 'other'.",
            "required" : False,
            "subAttributes" : emails_sub_attrs,
            "mutability" : "readWrite",
            "returned" : "default",
            "uniqueness" : "none"
        }
        user_account_attrs.append(emails)
        ...
        groups_sub_attrs.append(group_display)
        groups = {
            "name" : "groups",
            "type" : "complex",
            "multiValued" : True,
            ...
            "required" : False,
            "subAttributes" : groups_sub_attrs,
            "mutability" : "readOnly",
            "returned" : "default"
        }
        user_account_attrs.append(groups)

        entitlements_sub_attrs = []
        ...
        entitlements_sub_attrs.append(entitlements_display)
        entitlements = {
            "name" : "entitlements",
            "type" : "complex",
            "multiValued" : True,
            ...
            "required" : False,
            "subAttributes" : entitlements_sub_attrs,
            "mutability" : "readWrite",
            "returned" : "default"
        }
        user_account_attrs.append(entitlements)

        user_account_schema = {
            "id" : "urn:ietf:params:scim:schemas:core:2.0:User",
            "name" : "Users",
            "description" : "User Account",
            "endpoint": "/Users",   #SCIM 1
            "schema": "urn:ietf:params:scim:schemas:core:2.0",    #SCIM 1
            "attributes" : user_account_attrs,
            "meta" : {
                "resourceType" : "Schema",
        ...
        if schema_id == "urn:ietf:params:scim:schemas:core:2.0:User":
            ...
            return {
                "totalResults": 1,   #SCIM 1
                "schemas": ["urn:ietf:params:scim:schemas:core:2.0"],   #SCIM 1
  		"Resources": [user_account_schema]
        }
...
def executeLogin(self, **kwargs):
        ...
        return cherrypy.session.id, token, perms
...
class Login(LowDataAdapter):
    ...
    def POST(self, **kwargs):
        ...
        **Example response:**
        ...
        '''
        session_id, token, perms = executeLogin(self)
        return {'return': [{
            'token': session_id,
            'expire': token['expire'],
            'start': token['start'],
...
class API(object):
    ...
    def _setattr_url_map(self):
        ...
        if self.apiopts.get('enable_sessions', True) is False:
            url_blacklist = ['login', 'logout', 'minions', 'jobs', 'user']
        else:
            url_blacklist = []
